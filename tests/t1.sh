#!/bin/bash
# T1 — fund the senders S1-S4 (and confirm P3) from pn0's prefunded signer.
source "$(dirname "$0")/gas2500x-lib.sh"
begin_case "T01" "fund the senders"

# Funding list: S1..S3 need gas money for T2/T5/T6/T8/T20+ and S4 for T34's
# above-floor survivor (1 XDC each); P3 (pn3's own account, sender of T13's
# default-price tx) needs 10 XDC.
...
for spec in "TXGEN_KEY_1 1ether" "TXGEN_KEY_2 1ether" \
    "TXGEN_KEY_3 1ether" "TXGEN_KEY_4 1ether" "PRIVATE_KEY_3 10ether"; do
    read -r s value <<<"$spec"
...
for s in TXGEN_KEY_1 TXGEN_KEY_2 TXGEN_KEY_3 TXGEN_KEY_4 PRIVATE_KEY_3; do
    to=$(addr_of "$s")
    bal=$(rpc3 eth_getBalance "[\"$to\", \"latest\"]" | jq -r .)
    [ "$(hex2dec "$bal")" -gt 0 ] || fail_case "$s ($to) balance 0"
done
pass_case "S1/S2/S3/S4 funded"
