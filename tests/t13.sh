#!/bin/bash
# T13 — the tier-aware default gas price on the pre-fork tier: P3's
# transfer is signed locally with NO gas price, so the node fills in the
# tier-aware suggested price (12.5 gwei), #2516.
# (The post-fork half of this check is T50; P3 = pn3's own account,
# PRIVATE_KEY_3.)
source "$(dirname "$0")/gas2500x-lib.sh"
begin_case "T13" "the tier-aware default gas price (pre-fork tier)"
