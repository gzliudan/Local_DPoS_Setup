...
# pn3: the sweep drops only below-floor txs, so T21's above-floor pre-fork
# survivor (700 gwei, parked queued by T21) is still there — exactly one
# queued tx when the seed ran, zero otherwise
que=$(hex2dec "$(rpc "$RPC3" txpool_status | jq -r .result.queued)")
if [ -f /tmp/g2500-t21-hashes ]; then
    [ "$que" = "1" ] || fail_case "pn3 queued=$que, expected the 1 T21 survivor"
    hash=$(tail -n 1 /tmp/g2500-t21-hashes)
    pool_hashes_from "$(addr_of TXGEN_KEY_4)" | grep -q "$hash" \
        || fail_case "pn3's queued tx is not the T21 survivor"
    pass_case "queued=0 on pn0-pn2; pn3 keeps the T21 survivor (700 gwei)"
fi
