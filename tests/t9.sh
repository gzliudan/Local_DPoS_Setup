...
# wait for the pn3 RPC to come back (head3 prints -1 while the node is down)
...
n10=$(printf '%s' "$content" | jq '[.queued[][]? | select(.nonce == "0xa"))] | length' 2>/dev/null || \
     printf '%s' "$content" | jq '[.. | objects | select(.nonce? == "0xa")] | length')
