...
then
  ...
  ${PROJECT_DIR}/build/bin/$Bin_NAME init --datadir ${DATA_DIR} ./genesis/genesis.json
else
...
echo ${child_proc} > "${WORK_DIR}/sync-from-genesis.pid"

sleep 3
cat "${WORK_DIR}/sync-from-genesis.pid"
